...
class PHPWebSocket
{
	...
	const WS_MAX_CLIENTS_PER_IP = 115;
	...
	*/
	...
	function wsStartServer($host, $port) {
		...
		$nextPingCheck 	= microtime(1) + 0.1;
		$nextTick 		= microtime(1) + 0.1;
		while (isset($this->wsRead[0])) {
			$changed = $this->wsRead;
			$result = socket_select($changed, $write, $except, 0,16000);
			...
			elseif ($result > 0) {
				foreach ($changed as $clientID => $socket) {
					if ($clientID != 0) {
						...
						//this is the client socket (do stuff on the client)
						...
						//this is the server socket (do stuff on the server)
			...
			if ( microtime(1) >= $nextTick) {
				$this->tick();
				$nextTick = microtime(1) + $this->tickDelay/1000;
			}
			

			// if ( microtime(1) >= $nextPingCheck) {
			// 	$this->wsCheckIdleClients();
			// 	$nextPingCheck = microtime(1) + 1;
			// }
		}
	...
	function wsSendClientMessage($clientID, $opcode, $message) {
		...
		for ($i=0; $i<$frameCount; $i++) {
			...
			// print_r(PHP_EOL.$buffer.PHP_EOL);
			// send frame
			$socket = $this->wsClients[$clientID][0];
...
class WS extends PHPWebSocket{

	const NODATA 		= "<no data>";
	const EMIT_FULL 	= 2;
	const EMIT_LIST 	= 1;
	const EMIT_CURR		= 0;
	protected static $host,$port,$server,$current=0;

	/**
	 * Initialize Socket and register function to start the server
	...
	public static function init($host,$port){
		self::$host   = $host;
		self::$port   = $port;
		self::$server = new PHPWebSocket();
		register_shutdown_function(function(){
			self::$server->wsStartServer(self::$host, self::$port);
		});
	}
	 ...
	 * @param  integer $flag Flags:
	 *                       	self::EMIT_FULL(2) = to all clients into the server
	 *                       	self::EMIT_LIST(1) = to all clients except the one who sent the msg
	 *                       	self::EMIT_CURR(0) = only to the client who sent the message
	 */
	public static function emit($event,$data = self::NODATA,$flag = self::EMIT_FULL){
		$clients = self::$server->wsClients;
		$data    = [
			"e"	=> $event,
			"d"	=> $data
		];
		switch ($flag) {
			case 1:
				foreach ( $clients as  $id => $client )
					if ($id != self::$current )
						self::$server->wsSend($id, json_encode($data));
				break;
			...
			case 2:
				...
				break;
			default:
				 self::$server->wsSend($clients[self::$current]['info']['id'], json_encode($data));
				break;
		}
	}
	...
	public static function send($msg){
		...
		foreach ( $clients as  $client )
			self::$server->wsSend($client['info']['id'], json_encode($msg));
	}

	/**
	 * Binds a funtion when the server receives a new connection request 
	 * @param  [function] $func callback function for the request
	 */
	public static function onopen($func){
		$callback=function($client) use ($func){
			self::$current = $client;

			$ip = long2ip( self::$server->wsClients[$client][6] );
		...
		self::$server->bind('open', $callback);
	}
	/**
	 * Binds a funtion when the server receives a disconnection request 
	 * @param  [function] $func callback function for the request
	 */
	public static function onclose($func){
		$callback = function($client, $status) use ($func){
			self::$current 	= $client;
			$ip   			= long2ip( self::$server->wsClients[$client][6] );
			self::$server->wsClients[$client]["info"]["id"]   = $client;
			self::$server->wsClients[$client]["info"]["ip"]   = $ip;
			self::$server->wsClients[$client]["info"]["name"] = $client;
			$func(self::$server->wsClients[$client]["info"], $status);
		};
		(self::$server->bind('close', $callback));
	}

	/**
	 * Registers events 
	 * @param  [string] $event event name
	 * @param  [function] $func  callback for event
	 */
	public static function on($event,$func){
		$callback=function($client, $message, $messageLength) use($event,$func){
			self::$current = $client;
			$message       = json_decode($message,1);
			if($event == $message['e'])
				$func($message['d'],self::$server->wsClients[$client]['info'],$messageLength);
		};
		(self::$server->bind('message', $callback));

	}
	public static function onTick($delay,$func){
		self::$server->tickDelay 	= $delay;
		self::$server->tickFunc 	= $func;
	}
